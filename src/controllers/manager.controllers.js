const { Reservation, Dish, Service, DishType } = require('../models');
...
const createDish = async (req, res) => {
	try {
		const { name, description, price, image, dishTypeId, isDrink, unit } =
			req.body;
		console.log(
			'🚀 ~ file: manager.controllers.js:6 ~ createDish ~ isDrink:',
			isDrink,
		);
...
const createTypeDish = async (req, res) => {
	const { type, isDrinkType } = req.body;
	try {
		...
		return res.status(200).json({
			isSuccess: true,
			msg: isDrinkType
				? 'Tạo phân loại đồ uống thành công'
				: 'Tạo phân loại món thành công',
		});
	} catch (error) {
		console.dir(error);
		return res.status(500).json({
			isSuccess: false,
			msg: isDrinkType
				? 'Tạo phân loại đồ uống thất bại'
				: 'Tạo phân loại món thất bại',
		});
	}
};

const deleteTypeDish = async (req, res) => {
	const { typeDishId } = req.body;
	try {
		console.dir(req.body);

		await DishType.destroy({
			where: {
				dishTypeId: typeDishId,
			},
		});
		return res.status(200).json({
			isSuccess: true,
			msg: 'Xóa phân loại thành công',
		});
	} catch (error) {
		console.dir(error);
		return res.status(500).json({
			isSuccess: false,
			msg: 'Xóa phân loại thất bại',
		});
	}
};
